...
export default function Features() {
  return (
    <section id="benefits" className="scroll-mt-0 py-20 lg:scroll-mt-[-1.75rem] lg:py-28">
      <div className="section-shell">
        ...
        <div className="mt-12 grid grid-cols-1 items-stretch gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((feature, index) => (
            <article
              key={feature.title}
              className="group flex h-full flex-col rounded-2xl border border-line bg-panel p-6 transition-all duration-300 hover:-translate-y-1 hover:border-ember hover:bg-raised"
            >
              <div className="mb-4 inline-flex w-fit rounded-xl bg-void p-3 transition-transform duration-300 group-hover:scale-110">
                <FeatureIcon index={index} />
              </div>
              <h3 className="font-display text-xl font-bold uppercase tracking-wide">{feature.title}</h3>
